...
func (s *state) ConfirmLegacySIDH(partner contact.Contact) (
	id.Round, error) {
	return s.confirmLegacySIDH(partner, s.params.ConfirmTag)
}

func (s *state) confirmLegacySIDH(partner contact.Contact, serviceTag string) (
	id.Round, error) {
	...
	err := s.store.HandleReceivedRequestLegacySIDH(partner.ID,
		func(rr *store.ReceivedRequestLegacySIDH) error {
			...
			p := s.sessionParams
			_, err := s.e2e.AddPartnerLegacySIDH(partner.ID, partner.DhPubKey, dhPriv,
...
func sendAuthConfirmLegacySIDH(net cmixClient, partner *id.ID,
