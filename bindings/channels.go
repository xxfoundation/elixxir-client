...
// SendGenericDM is used to send a raw message via DM. In general, it
// should be wrapped in a function that defines the wire protocol. If the final
// message, before being sent over the wire, is too long, this will return an
// error. Due to the underlying encoding using compression, it isn't possible to
// define the largest payload that can be sent, but it will always be possible
...
// Returns:
//   - []byte - A JSON marshalled ChannelSendReport.
func (cm *ChannelsManager) SendGenericDM(messageType int, partnerPubKeyBytes,
	dmToken, message []byte, leaseTimeMS int64,
	cmixParamsJSON []byte) ([]byte, error) {
	...
	// Send message
	chanMsgId, rnd, ephId, err := cm.api.SendGenericDM(&partnerPubKey,
...
// SendDM is used to send a formatted direct message.
//
// Parameters:
//   - partnerPubKeyBytes - the bytes of the public key of the partner's ed25519
//     signing key.
...
// Returns:
//   - []byte - A JSON marshalled ChannelSendReport
func (cm *ChannelsManager) SendDM(partnerPubKeyBytes, dmToken []byte,
	...
	// Send message
	chanMsgId, rnd, ephId, err := cm.api.SendDM(&partnerPubKey, dmToken,
		message, params.CMIX)
	if err != nil {
		return nil, err
	}
...
// SendDMReply is used to send a formatted direct message.
//
// If the message ID the reply is sent to is nonexistent, the other side will
// post the message as a normal message and not a reply. The message will auto
// delete validUntil after the round it is sent in, lasting forever if
...
// Returns:
//   - []byte - A JSON marshalled ChannelSendReport
func (cm *ChannelsManager) SendDMReply(partnerPubKeyBytes, dmToken []byte,
	message string, messageToReactTo []byte, leaseTimeMS int64,
	cmixParamsJSON []byte) ([]byte, error) {
	...
	// Send Reply
	chanMsgId, rnd, ephId, err := cm.api.SendDMReply(&partnerPubKey, dmToken,
...
// SendDMReaction is used to send a reaction to a direct message
// The reaction must be a single emoji with no other characters, and will
// be rejected otherwise.
// Users will drop the reaction if they do not recognize the reactTo message.
//
...
// Returns:
//   - []byte - A JSON marshalled ChannelSendReport.
func (cm *ChannelsManager) SendDMReaction(partnerPubKeyBytes, dmToken []byte,
	reaction string, messageToReactTo []byte,
	cmixParamsJSON []byte) ([]byte, error) {
	...
	// Send reaction
	chanMsgId, rnd, ephId, err := cm.api.SendDMReaction(&partnerPubKey,
...
type EventModel interface {
	...
	//  - messageID - The bytes of the [channel.MessageID] of the received
	//    message.
	//  - reactionTo - The [channel.MessageID] for the message
...
// ReceiveDM is called whenever a direct message is received.
// It may be called multiple times on the same message. It is incumbent on the
// user of the API to filter such called by message ID.
...
// ReceiveDMReply is called whenever a direct message is received that
...
// ReceiveDMReaction is called whenever a reaction to a direct message
