...
type ChannelExtentionHander interface {
}
...
func LoadChannelsManager(cmixID int, storageTag string,
	...
	m, err := channels.LoadManagerBuilder(storageTag, user.api.GetStorage().GetKV(),
		user.api.GetCmix(), user.api.GetRng(), eb, nil)
	if err != nil {
		return nil, err
	}
...
func NewChannelsManagerGoEventModel(cmixID int, privateIdentity []byte,
	goEventBuilder channels.EventModelBuilder, builders []channels.ExtensionBuilder) (*ChannelsManager, error) {
	pi, err := cryptoChannel.UnmarshalPrivateIdentity(privateIdentity)
	if err != nil {
		return nil, err
	}
...
func LoadChannelsManagerGoEventModel(cmixID int, storageTag string,
	...
	// Construct new channels manager
	m, err := channels.LoadManagerBuilder(storageTag, user.api.GetStorage().GetKV(),
		user.api.GetCmix(), user.api.GetRng(), goEventBuilder, builders)
	if err != nil {
		return nil, err
	}
