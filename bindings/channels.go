...
//
//export GenerateChannelIdentity
...
//
//export GetPublicChannelIdentity
...
//
//export GetPublicChannelIdentityFromPrivate
...
//
//export NewChannelsManagerMobile
func NewChannelsManagerMobile(cmixID int, privateIdentity []byte,
	...
	m, err := channels.NewManager(pi, user.api.GetStorage().GetKV(),
		user.api.GetCmix(), user.api.GetRng(), model, user.api.AddService)
	if err != nil {
		return nil, err
	}
...
//   - muteCb - Callback that is invoked whenever a sender is muted/unmuted.
//
//export LoadChannelsManagerMobile
func LoadChannelsManagerMobile(cmixID int, storageTag string,
	...
	m, err := channels.LoadManager(storageTag, user.api.GetStorage().GetKV(),
		user.api.GetCmix(), user.api.GetRng(), model)
	if err != nil {
		return nil, err
	}
...
func LoadChannelsManager(cmixID int, storageTag string,
	...
	// Construct new channels manager
	m, err := channels.LoadManagerBuilder(storageTag, user.api.GetStorage().GetKV(),
...
//
//export NewChannelsManagerGoEventModel
...
//     not compatible with GoMobile bindings.
//
//export LoadChannelsManagerGoEventModel
...
//
//export GetChannelDbCipherTrackerFromID
...
//
//export NewChannelsDatabaseCipher
