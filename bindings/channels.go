...
func NewChannelsManagerMobile(cmixID int, privateIdentity []byte,
	dbFilePath string, cipherID int, extensionBuilderIDsJSON []byte,
...
//   - deleteCb - Callback that is invoked whenever a message is deleted.
//   - muteCb - Callback that is invoked whenever a sender is muted/unmuted.
func LoadChannelsManagerMobile(cmixID int, storageTag string, dbFilePath string,
	cipherID int, extensionBuilderIDsJSON []byte, msgCb MessageReceivedCallback,
	deleteCb DeletedMessageCallback, muteCb MuteCallback) (
...
func LoadChannelsManagerGoEventModel(cmixID int, storageTag string,
	...
	*ChannelsManager, error) {
	...
	m, err := channels.LoadManagerBuilder(storageTag,
		user.api.GetStorage().GetKV(), user.api.GetCmix(), user.api.GetRng(),
		goEventBuilder, extensionBuilders)
	if err != nil {
		return nil, err
	}
