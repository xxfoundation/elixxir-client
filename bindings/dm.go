...
//   - nuCB - A callback that is triggered everytime there is a change to the
//     notification status of a DM conversation.
func NewDMClient(cmixID, notificationsID int, privateIdentity []byte,
	receiverBuilder DMReceiverBuilder, nuCB DmNotificationUpdate) (*DMClient, error) {
	...
	m, err := dm.NewDMClient(&pi, receiver, sendTracker, nickMgr, nm.manager,
		user.api.GetCmix(), dmKV, user.api.GetRng(), wrapDmNotificationUpdate(nuCB))
	if err != nil {
		return nil, err
	}
...
func NewDMClientWithGoEventModel(cmixID, notificationsID int,
	privateIdentity []byte, receiver dm.EventModel, nuCB DmNotificationUpdate) (*DMClient, error) {
	pi, err := codename.UnmarshalPrivateIdentity(privateIdentity)
	if err != nil {
		return nil, err
	}
...
func NewDmManagerMobile(cmixID, notificationsID int, privateIdentity []byte,
	dbFilePath string, msgCb DmReceivedCallback, nuCB DmNotificationUpdate) (
	*DMClient, error) {
...
func wrapDmNotificationUpdate(cb DmNotificationUpdate) dm.NotificationUpdate {
	return func(nf dm.NotificationFilter,
		...
		cb.Callback(nfJSON, changedStateListJSON, deletedListJSON)
	}
}