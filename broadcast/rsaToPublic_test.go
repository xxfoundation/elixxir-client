...
func Test_asymmetricClient_Smoke(t *testing.T) {
	...
	channel, pk, _ := crypto.NewChannel(cName, cDesc, crypto.Public, false,
		packetPayloadLength, rngGen.GetStream())
	...
	for i := range clients {
		...
		_, err = s.RegisterRSAtoPublicListener(cb, []string{})
		if err != nil {
	...
	for i := range clients {
		...
		// Broadcast payload
		_, _, _, err := clients[i].BroadcastRSAtoPublic(
			pk, payload, []string{}, dummyMessageType, cmix.GetDefaultCMIXParams())
		if err != nil {
	...
	// Broadcast payload
	_, _, _, err := clients[0].
		BroadcastRSAtoPublic(pk, payload, []string{}, dummyMessageType, cmix.GetDefaultCMIXParams())
	if err != nil {
