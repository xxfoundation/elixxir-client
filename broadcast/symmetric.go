...
func (bc *broadcastClient) Broadcast(payload []byte, tags []string,
	metadata [2]byte, cMixParams cmix.CMIXParams) (rounds.Round, ephemeral.Id, error) {
	assemble := func(rid id.Round) ([]byte, error) {
		return payload, nil
	}
	return bc.BroadcastWithAssembler(assemble, tags, metadata, cMixParams)
}
...
func (bc *broadcastClient) BroadcastWithAssembler(assembler Assembler,
	...
	assemble := func(rid id.Round) (fp format.Fingerprint,
		...
		service = bc.GetSymmetricCompressedService(tags, metadata)
...
func (bc *broadcastClient) GetSymmetricCompressedService(
