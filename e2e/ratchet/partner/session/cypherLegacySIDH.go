...
// results.
func GenerateE2ESessionBaseKeyLegacySIDH(myDHPrivKey, theirDHPubKey *cyclic.Int,
...
// CypherLegacySIDH manages the cryptographic material for E2E messages and provides
...
// cypherLegacySIDH adheres to the CypherLegacySIDH interface.
type cypherLegacySIDH struct {
	session *SessionLegacySIDH

	fp *format.Fingerprint

	// keyNum is the index of the key by order of creation. It is used to
	// identify the key in the cypherLegacySIDH.SessionLegacySIDH.
	keyNum uint32
}

func newCypherLegacySIDH(session *SessionLegacySIDH, keyNum uint32) *cypherLegacySIDH {
	return &cypherLegacySIDH{
		session: session,
		keyNum:  keyNum,
	}
}
...
// GetSessionLegacySIDH return pointers to higher level management structures.
func (k *cypherLegacySIDH) GetSession() *SessionLegacySIDH { return k.session }

// Fingerprint returns the CypherLegacySIDH key fingerprint, if it has it. Otherwise, it
...
func (k *cypherLegacySIDH) Fingerprint() format.Fingerprint {
	if k.fp != nil {
		return *k.fp
	}
...
func (k *cypherLegacySIDH) Encrypt(contents []byte) (ecrContents, mac []byte, residue e2eCrypto.KeyResidue) {
	fp := k.Fingerprint()
	key := k.generateKey()
...
func (k *cypherLegacySIDH) Decrypt(msg format.Message) (decryptedPayload []byte, residue e2eCrypto.KeyResidue, err error) {
	fp := k.Fingerprint()
	key := k.generateKey()
