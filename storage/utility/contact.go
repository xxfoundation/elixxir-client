...
func StoreContact(kv *KV, c contact.Contact) error {
	now := netTime.Now()
	...
	return kv.Set(makeContactKey(c.ID), obj.Marshal())
}
...
func LoadContact(kv *KV, cid *id.ID) (contact.Contact, error) {
	vo, err := kv.Get(makeContactKey(cid), currentContactVersion)
	if err != nil {
		return contact.Contact{}, err
	}

	return contact.Unmarshal(vo)
}
...
// Returns:
//   - error if an error occurs or nil otherwise
func DeleteContact(kv *KV, cid *id.ID) error {
